...
Requirements:
pip install llama-cpp-python pandas openpyxl pyodbc sqlalchemy tqdm
...
CATEGORY_FILE   = r”C:\data\SPSS_Category_Concepts.xlsx”   # Image 1
INTENT_FILE     = r”C:\data\Email_Intents.xlsx”             # Image 2

# SQL Server connection — uses Windows Auth (Trusted_Connection)

# Switch to UID=…;PWD=… if using SQL auth

SQL_SERVER      = “GENESYSCICDBQA.corp.hyd…”              # from Image 3
SQL_DATABASE    = “EMAIL”
SQL_TABLE       = “[dbo].[Original_Email]”
SQL_TRUSTED     = True          # set False if using SQL auth
SQL_UID         = “”            # only needed if SQL_TRUSTED = False
SQL_PWD         = “”

# Prediction behaviour

BATCH_SIZE      = 10            # emails per run (increase after testing)
CONFIDENCE_COL  = True          # add a comment with confidence to Predicted
DRY_RUN         = False         # True = predict but do NOT write to DB
N_GPU_LAYERS    = 0             # set >0 if you have a CUDA GPU
N_CTX           = 4096          # context window (tokens)
MAX_TOKENS      = 100           # max tokens in LLM response
...
def load_category_file(path: str) -> dict[str, list[str]]:
“””
...
def load_intent_file(path: str) -> tuple[list[str], list[dict]]:
“””
...
```
intent_col = "Intent"
code_col   = "Intent_Code" if "Intent_Code" in df.columns else "Intent_Code"
email_col  = "Email"
...
url = f”mssql+pyodbc:///?odbc_connect={cs}”
...
def write_predictions(engine, predictions: list[dict]):
“””
...
def parse_llm_response(raw: str, valid_codes: list[str]) -> dict:
“””
...
if len(sys.argv) > 1 and sys.argv[1] == "test":
    # python email_intent_predictor.py test
    test_single_email()
else:
    # python email_intent_predictor.py
    main()
```
